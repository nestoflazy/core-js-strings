...
function removeFirstOccurrences(str, value) {
  ...
  const second = str.slice(str.indexOf(value) + value.length);
  return first + second;
}
...
function removeLastOccurrences(str, value) {
  ...
  const first = str.slice(0, str.lastIndexOf(value));
  const second = str.slice(str.lastIndexOf(value) + value.length);
...
function countVowels(str) {
  ...
  return str
    .split('')
    .reduce((acc, cur) => acc + (Vowels.includes(cur) ? 1 : 0), 0);
}
...
function isPalindrome(str) {
  const base = str.toLowerCase().replaceAll(/[\W_]+/g, '');
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((el) => (base.includes(el) ? ROT13[base.indexOf(el)] : el))
    .join('');
}
...
function getCardId(value) {
  ...
  const suit = ['♣', '♦', '♥', '♠'];
  const index1 = deck.indexOf(value.slice(0, -1));
  const index2 = suit.indexOf(value.slice(-1));

  return index1 + deck.length * index2;
}
